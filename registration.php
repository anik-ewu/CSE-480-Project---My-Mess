...
function checkUserName($uname){     //Check user name to avoid the duplicate user name.
    ...
    $sql = "select user_name from manager where user_name = '$uname'";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    else
    {
        if($email == true)
        {
            ...
            $sql = "insert into manager(`user_name`,`full_name`,`email`,`password`,`phone`,`mess_id`) values('$username','$fullname','$email','$password','$phone','$mid')";
